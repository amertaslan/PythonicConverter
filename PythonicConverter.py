...
def CSV2XML():
    reader = csv.reader(open('DEPARTMENTS.csv', 'r', encoding='utf-8'), delimiter=';')
    next(reader)
...
def XML2CSV():
    xmlFile = open('formattedXml.xml', 'r', encoding='utf-8')
    tree = ET.parse(xmlFile)
    root = tree.getroot()
    csvfile = open('xmltocsvfile2.csv', 'w', encoding='utf-8', newline='')
...
def CSV2JSON():
    ...
    csv_file = csv.DictReader(open('DEPARTMENTS.csv', 'r', encoding='utf-8'), delimiter=';')

    jsonfile = open('jsondeneme.json', 'w', encoding='utf-8')
...
def JSON2CSV():
    jsonFile = open('jsondeneme.json', encoding='utf-8')
    data = json.loads(jsonFile.read())
    csvFile = open('jsontocsvfile.csv', 'w', encoding='utf-8', newline='')
    fieldnames = ['ÜNİVERSİTE_TÜRÜ', 'ÜNİVERSİTE', 'FAKÜLTE', 'PROGRAM_KODU', 'PROGRAM', 'DİL', 'ÖĞRENİM_TÜRÜ', 'BURS', 'ÖĞRENİM_SÜRESİ',
                  'PUAN_TÜRÜ', 'KONTENJAN', 'OKUL_BİRİNCİSİ_KONTENJANI', 'GEÇEN_YIL_MİN_SIRALAMA', 'GEÇEN_YIL_MİN_PUAN']
    writer = csv.DictWriter(csvFile, fieldnames=fieldnames, delimiter=';')
    writer.writeheader()
...
def validateXSD():
    doc = etree.parse('jsontoxml.xml')
    root = doc.getroot()
    #print(etree.tostring(root))
    xmlschema_doc = etree.parse('validationXSD.xsd')
